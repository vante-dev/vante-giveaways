...
class Giveaway extends EventEmitter {
    ...
    /**
     * The button on the giveaway message.
     */
    get messageButton() {
        return (
            this.message?.fetch({ force: true }).then(message => message.components[0].components[0])
    ...
    get data() {
        return {
            ...
            emoji: this.options.buttonEmoji,
    ...
    /**
     * Fetches all users of the giveaway, except bots, if not otherwise specified.
     * @returns {Promise<Discord.Collection<Discord.Snowflake, Discord.User>>} The collection of Participants
     */
    async fetchAllEntrants() {
        return new Promise(async (resolve, reject) => {
            try {
                const giveaway = this.manager.giveaways.filter((g) => g.messageId === this.messageId)[0];
                ...
                while (userCollection.size % 100 === 0) {
                    const newUsers = await this.client.users.fetch({ after: userCollection.lastKey() });
                ...
                reject('Unable to fetch the participants: ' + error);
            }
        });
    }
     ...
     */
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (winners.length > 0) {
                ...
                .addComponents(new Discord.ButtonBuilder()
                    .setLabel(`${this.messages.giveawayEndedButton}`)
                ...
                if (this.messages.winMessage.embed && typeof this.messages.winMessage.embed === 'object') {
                    ...
                    if (embedDescription.length <= 4096) {
                        ...
                        if (Discord.embedLength(lastEmbed.data)) {
                            channel.send({ embeds: [lastEmbed], allowedMentions: this.allowedMentions, components: [row], });
                        }
                    }
                ...
                if (options.messages.congrat.embed && typeof options.messages.congrat.embed === 'object') {
                    ...
                    if (embedDescription.length <= 4096) {
                        ...
                        if (Discord.embedLength(lastEmbed.toJSON())) {
                            channel.send({ embeds: [lastEmbed], components: [row], allowedMentions: this.allowedMentions });
                        }
                    }
                } else if (message?.length <= 2000) {
                    const row = new Discord.ActionRowBuilder()
    ...
    unpause() {
        return new Promise(async (resolve, reject) => {
            ...
            const lastChanceEnabled =
                this.lastChance.enabled && this.remainingTime < this.lastChance.threshold;
            const buttonClickedEnabled = 
                this.lastChance.enabled && this.participants.length > 0
